...
@Injectable()
export class SearchCategoryProvider {
    private readonly logger: Logger = new Logger(SearchCategoryProvider.name);
    ...
    async getCoursesWithCategory(category: string, searchQueryDto: SearchQueryDto) {
        const Number_of_learners = await this.numberOfStudentsEnrolledWithTag(category);
        const statistics = await this.numberOfCoursesWithTag(category);
        const instructors = await this.instructorData(category);
    ...
    private async numberOfCoursesWithTag(category: string) {
        try {
    ...
    private async numberOfStudentsEnrolledWithTag(category: string) {
        try {
    ...
    /*
     instructor name, id
     */
    private async instructorData(category: string) {
        try {
    ...
    /*
     ...
     number of lectures
     */
    private async coursesWithMetaData(category: string, searchQueryDto: SearchQueryDto) {
        try {
