...
export class FileService {
    ...
    async uploadImage(file: Express.Multer.File, folder: string): Promise<string> {
        const processed_image = await this.imageProcessingProvider.resizeAndOptimize(
            file,
            200,
            200,
        );
        ...
        return `https://${this.configService.awsConfig.cloudfront_url}/${s3ResponseKey}`;
    }
    ...
    private generate_filename(file: Express.Multer.File, folder: string): string {
        ...
        return `${folder}/${name}-${timestamp}-${uuid4()}${extension}`;
    }
}
